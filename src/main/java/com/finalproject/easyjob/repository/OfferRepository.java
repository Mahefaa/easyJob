...
public interface OfferRepository extends JpaRepository<Offer, Integer> {
  @Query("select o from Offer o where upper(o.status) like upper(:filter) or o.sender.email like %:filter% or lower(o.domain.name) like lower(concat('%',:filter,'%')) or lower(o.position) like lower(concat('%',:filter,'%')) or lower(o.mission) like lower(concat('%',:filter,'%')) or lower(o.profile) like lower(concat('%',:filter,'%')) or lower(o.location) like lower(concat('%',:filter,'%') )")
